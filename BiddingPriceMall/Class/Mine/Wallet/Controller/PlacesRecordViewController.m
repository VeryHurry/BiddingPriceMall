...
@implementation PlacesRecordViewController
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"PlacesRecordCell" bundle:nil] forCellReuseIdentifier:@"place_cell"];
...
-(void)getPlacesList
{
    ...
    [[XXNetWorkMangerBase sharedNetWorkManger] postWithUrl:api_myPlacesRecord andData:dic andSuccessBlock:^(id success) {
        self.model = [PlacesListModel modelWithJSON:success];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PlacesRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:@"place_cell" forIndexPath:indexPath];
...
- (CQMenuTabView *)menuView
{
    if (!_menuView) {
        ...
        _menuView.titles = @[@"全部",@"配送仓单",@"辅导配售",@"分享配售",@"代理配售",@"仓单买入"];
    }
    return _menuView;
}
...
-(NSArray *)getCurrentArray:(NSInteger)count type:(NSInteger)type
{
    ...
    for (int i = 0; i <count; i ++) {
        PlacesModel *model = self.capitalArr[i];
        //买入普通
        if (type == 0)
        {
            if (model.placesChannel == 1) {
